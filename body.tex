...
\item 2-band rolling cadence at 50\% strength % rolling
\item 2-band rolling cadence at 90\% strength % rolling
\item 3-band rolling cadence at 50\% strength % rolling
\item 3-band rolling cadence at 90\% strength % rolling
\item 6-band rolling cadence at 50\% strength % rolling_six
\item 6-band rolling cadence at 90\% strength % rolling_six
\item 2-band rolling cadence, starting earlier than the baseline so an additional rolling season can be completed % roll_early
\item Rolling also in the bulge (with the bulge divided in half) % rolling_bulge
\item ROlling in the bulge and the high extinction galactic plane % rolling_all_sky
\item Rolling in the bulge with 6 stripes % rolling_bulge_6
\end{itemize}
