...
As a follow up to the Fall 2020 LSST Cadence Optimization Report and subsequent simulated surveys analysis by the LSST Science Collaborations, the SCOC solicited and received 39 Cadence Notes in April 2021 \citep{Document-37579}. The call for Cadence Notes \citep{Document-37228} aimed to fill additional gaps in metric coverage and get feedback on the existing large set of simulations, based on responses to seven specific questions. The summary SCOC  findings for each of these questions and resulting SCOC recommendations are listed below, followed by a detailed description of the next generation of survey simulations derived from these recommendations.

Links:
\begin{itemize}
\item LSST Science Requirements Document,  \citep{LPM-17}
\item 2018 Cadence White Paper Submissions (\url{https://www.lsst.org/submitted-whitepaper-2018}, also available as a single download in \citet{Document-30641})
\item Fall 2020 LSST Cadence Optimization Report \citep{PSTN-051}
\item 2020 Call for Cadence Notes \citep{Document-37228}
\item 2021 Cadence Notes Submissions (\url{https://www.lsst.org/content/survey-cadence-notes-2021}, also available as a single download in \citet{Document-37579})
\end{itemize}
...
\begin{figure*}[htb!]
\begin{center}
\plotone{baseline_v2_0_10yrs_Nvisits_HEAL_SkyMap}
...
\tabletypesize{\footnotesize} % adjust or remove as needed
\tablewidth{0pt} % natural width; AASTeX will size columns
\tablecaption{Summary of Simulation Families\label{tab:shortlist}}
\tablehead{
\colhead{Family} & \colhead{Brief Description} & \colhead{\# sims.}
}
\startdata
Baseline & Updated survey footprint, 2$\times$15s visits in $grizy$ and 1$\times$30s in $u$, 2-band rolling in WFD. & 1 \\
Retro comparison & A comparison bridge point between v1.X and v2.0 & 2 \\
Rolling cadence & Variations on the rolling cadence & 13 \\
Longer $u$ visits & Extend $u$-band visit time to 1$\times$50s & 2 \\
